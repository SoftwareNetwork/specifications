...
{
    auto &openssl = s.addProject("openssl", "1.1.1.22");
    {
        auto v = "OpenSSL_{M}_{m}_{p}"s;
        v += (char)('a' - 1 + openssl.getPackage().getVersion().getTweak());
        openssl += Git("https://github.com/openssl/openssl", v);
    }
    ...
    auto &crypto = openssl.addTarget<LibraryTarget>("crypto");
    {
        ...
        crypto.Variables["SW_SHARED_LIBRARY_SUFFIX"] = crypto.getBuildSettings().TargetOS.getSharedLibraryExtension();
...
/*
    ...
    # define DSO_EXTENSION "${SW_SHARED_LIBRARY_SUFFIX}"
        ...
        crypto.configureFile(crypto.BinaryDir / "internal/bn_conf.h.in", "crypto/bn_conf.h");
        crypto.configureFile(crypto.BinaryDir / "internal/dso_conf.h.in", "crypto/dso_conf.h");
    }
