...
{
    auto &pqxx = s.addTarget<LibraryTarget>("jtv.pqxx", "6.3.0");
    ...
    pqxx.writeFileOnce("pqxx/config-public-compiler.h", "#include <iso646.h>");
    pqxx.writeFileOnce("pqxx/config-internal-libpq.h");
    pqxx.writeFileOnce("pqxx/config-internal-compiler.h");

    pqxx.patch("include/pqxx/internal/encodings.hxx", "CALLBACK", "CB");
}
