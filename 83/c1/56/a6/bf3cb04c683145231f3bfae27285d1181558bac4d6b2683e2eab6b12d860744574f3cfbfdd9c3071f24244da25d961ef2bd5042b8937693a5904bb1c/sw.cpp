...
#undef HANDLE_EXTENSION)");
    ...
    auto add_arch = [&](const std::string &dir)
    {
        ...
        auto &ACodeGen = d.template addTarget<StaticLibraryTarget>("CodeGen");
        {
            ...
            if (dir == "AArch64" || dir == "PowerPC" || dir == "RISCV") {
...
void build_clang(auto &&llvm_project, LlvmTargets &targets)
{
    ...
    auto &Basic = clang.template addTarget<StaticLibraryTarget>("Basic");
    {
        ...
        clang_tablegen(Basic, "include/clang/Basic/Attr.td",
            "clang/Basic/AttrTokenKinds.inc", { "-gen-clang-attr-token-kinds" }, true);
    ...
    libclang.Public += Clang_CodeGen, FrontendTool, Index, Tooling, ExtractAPI;
    ...
    clang_clang.Public += Clang_CodeGen, FrontendTool, Index, Tooling, RewriteFrontend, ExtractAPI;
    for (auto &[n, v] : targets.arch_targets)
    {
        for (auto &t : v)
...
{
    project<sw::ProjectTarget> llvm_project{s.addProject("llvm_project", "17.0.2")};
