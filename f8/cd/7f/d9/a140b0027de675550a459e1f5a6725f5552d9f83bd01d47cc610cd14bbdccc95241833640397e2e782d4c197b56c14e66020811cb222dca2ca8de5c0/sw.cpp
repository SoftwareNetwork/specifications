...
{
    auto &p = s.addProject("google.angle", "test004269");
    p += Git("https://chromium.googlesource.com/angle/angle", "", ""/*"chromium/{p}"*/, "f5dace0f1e57");
    ...
    auto &angle = p.addTarget<StaticLibraryTarget>("angle");
    {
        ...
        angle -=
            ...
            //"src/libANGLE/renderer/gl/egl/gbm/.*"_rr,
            "src/libANGLE/renderer/gl/egl/ozone/.*"_rr,
            "src/libANGLE/renderer/gl/eagl/.*"_rr, // ios
            "src/libANGLE/renderer/gl/glx/.*"_rr,
            "src/libANGLE/renderer/vulkan/.*"_rr;
        angle -= "src/libANGLE/renderer/gl/egl/DmaBufImageSiblingEGL.cpp";
        ...
        angle += "org.sw.demo.chromium.third_party.zlib.google.compression_utils_portable-master"_dep;

        angle -= "src/libANGLE/renderer/glslang_wrapper_utils.cpp";
        //angle += "org.sw.demo.KhronosGroup.glslang-master"_dep;

        angle.writeFileOnce("commit.h", R"(
#define ANGLE_COMMIT_HASH "0"
#define ANGLE_COMMIT_HASH_SIZE 1
