...
{
    auto &lzma = s.addTarget<LibraryTarget>("xz_utils.lzma", "5.2.10");
    ...
    if (lzma.getBuildSettings().TargetOS.Type == OSType::Windows ||
        lzma.getBuildSettings().TargetOS.Type == OSType::Mingw
        )
    {
    ...
    lzma.patch("src/liblzma/check/check.h",
        "#ifdef HAVE_IMMINTRIN_H",
        "#if defined(HAVE_IMMINTRIN_H) && !defined(__aarch64__)"
    );
