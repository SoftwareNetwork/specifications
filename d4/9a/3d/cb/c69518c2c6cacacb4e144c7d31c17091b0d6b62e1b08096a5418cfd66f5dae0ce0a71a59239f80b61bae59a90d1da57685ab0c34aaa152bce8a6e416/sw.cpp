...
{
    auto &p = s.addProject("gnome.glib", "2.88.3");
    ...
    auto &glib = p.addTarget<LibraryTarget>("glib");
    {
        ...
        glib -= "glib/gspawn-unsupported.c";
        ...
        // for clang-cl?
        if (glib.getBuildSettings().TargetOS.Type == OSType::Windows || glib.getBuildSettings().TargetOS.Type == OSType::Mingw) {
            glib.Protected += "UNICODE"_def;
            glib["glib/dirent/dirent.c"].args.push_back("-UUNICODE");
        }
        ...
        if (glib.getBuildSettings().TargetOS.Type != OSType::Windows &&
            glib.getBuildSettings().TargetOS.Type != OSType::Mingw &&
            !glib.getBuildSettings().TargetOS.isApple())
        {
            glib.Variables["gint64"] = "long";
        } else {
...
inline int gettimeofday(struct timeval * tp, struct timezone * tzp)
{
    ...
    return 0;
        ...
        gio.patch("gio/gio.h", "#include <gio/gmemorymonitor.h>", "#include <gio/gmemorymonitorbase.h>\n#include  <gio/gmemorymonitor.h>");
...
void check(Checker &c)
{
    ...
    s.checkIncludeExists("netlink/netlink.h");
    s.checkIncludeExists("netlink/netlink_route.h");
