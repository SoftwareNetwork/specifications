...
{
    auto &hpx = s.addTarget<SharedLibraryTarget>("StellarGroup.hpx", "1.3.0");
    hpx += Git("https://github.com/STEllAR-GROUP/hpx", "{v}");

    auto &pp = hpx.addStaticLibrary("preprocessor");
    pp.setRootDirectory("libs/preprocessor");
    ...
    //
    {
        ...
        hpx.Public += "org.sw.demo.boost.filesystem-1"_dep;
        ...
        hpx.Public += "org.sw.demo.open_mpi.hwloc"_dep;
    ...
    auto add_comp = [&add_plugin](const String &name) -> decltype(auto)
    {
        auto d = name;
        boost::replace_all(d, ".", "/");
        return add_plugin("components." + name, "src/components/" + d);
    };
