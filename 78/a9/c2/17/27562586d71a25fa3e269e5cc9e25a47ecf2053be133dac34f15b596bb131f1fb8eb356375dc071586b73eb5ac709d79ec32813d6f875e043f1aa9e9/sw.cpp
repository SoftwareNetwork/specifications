...
template <typename T>
T &add_target(auto &parent, auto &&...args) {
    auto setup_target = [](auto &t) {
        t += cpp20;
    };
    auto &t = parent.template addTarget<T>(args...);
    setup_target(t);
    return t;
}
...
-we4238 // Promote 'nonstandard extension used : class rvalue used as lvalue' to error.
    ...
    */
    ...
    auto &siphash = llvm_project.template addTarget<StaticLibraryTarget>("third_party.siphash");
    siphash += "third-party/siphash/.*"_rr;
    siphash.Public += "third-party/siphash/include"_idir;
...
#undef HANDLE_EXTENSION)");
        ...
        if (Support.getBuildSettings().TargetOS.Type == OSType::Windows)
        {
            ...
            Support += "uuid.lib"_slib;
            Support += "ntdll.lib"_slib;
        }
    ...
    auto &TablegenBasic = Tablegen.template addTarget<ObjectLibraryTarget>("Basic");
    TablegenBasic += "utils/TableGen/Basic/.*"_r;
    TablegenBasic.Public += Tablegen;

    //
    auto &UMinTablegen = llvm.template addTarget<ExecutableTarget>("Utils.MinTablegen");
    UMinTablegen += "utils/TableGen/llvm-min-tblgen.cpp";
    UMinTablegen.Public += TablegenBasic;
    ...
    UTablegen += "utils/TableGen/.*"_r;
    UTablegen += "utils/TableGen"_idir;
    UTablegen += "utils/TableGen/Common/.*"_rr;
    UTablegen -= "utils/TableGen/llvm-min-tblgen.cpp";
    UTablegen -= "utils/TableGen/.*\\.def"_rr;
    //UTablegen += "lib/Target/X86/Disassembler/X86DisassemblerDecoderCommon.h";
    UTablegen.Public += TablegenBasic, min_gen;
    ...
    auto &intrinsics_gen = llvm.template addTarget<StaticLibraryTarget>("intrinsics_gen");
    {
        ...
        tablegen2("RuntimeLibcalls.td", "RuntimeLibcalls.inc", { "-gen-runtime-libcalls" });
        ...
        tablegen2("Intrinsics.td", "IntrinsicsSPIRV.h", { "-gen-intrinsic-enums", "-intrinsic-prefix=spv" });
    ...
    tablegen(Analysis, "include/llvm/Analysis/TargetLibraryInfo.td", "llvm/Analysis/TargetLibraryInfo.inc", {"-gen-target-library-info"});
    ...
    auto &FrontendOpenACC = llvm.template addTarget<StaticLibraryTarget>("Frontend.OpenACC");
    FrontendOpenACC += "lib/Frontend/OpenACC/.*"_rr;
    auto &FrontendOpenACCGen = llvm.template addTarget<StaticLibraryTarget>("Frontend.OpenACC.Gen");
    tablegen(FrontendOpenACCGen, "include/llvm/Frontend/OpenACC/ACC.td", "llvm/Frontend/OpenACC/ACC.inc", { "-gen-directive-impl" });
    tablegen(FrontendOpenACCGen, "include/llvm/Frontend/OpenACC/ACC.td", "llvm/Frontend/OpenACC/ACC.h.inc", { "-gen-directive-decl" });
    FrontendOpenACC.Public += Core, FrontendOpenACCGen;
    ...
    tablegen(TargetParser, "lib/Target/ARM/ARM.td", "llvm/TargetParser/ARMTargetParserDef.inc", {"-gen-arm-target-def"}, true);
    tablegen(TargetParser, "lib/Target/AArch64/AArch64.td", "llvm/TargetParser/AArch64TargetParserDef.inc", {"-gen-arm-target-def"}, true);
    tablegen(TargetParser, "lib/Target/RISCV/RISCV.td", "llvm/TargetParser/RISCVTargetParserDef.inc", {"-gen-riscv-target-def"}, true);
    tablegen(TargetParser, "lib/Target/PowerPC/PPC.td", "llvm/TargetParser/PPCGenTargetFeatures.inc", {"-gen-target-features"}, true);
    ...
    Passes.Public += AggressiveInstCombine, Analysis, CodeGen
        //, HelloWorldPass
...
void build_clang(auto &&llvm_project, LlvmTargets &targets)
{
    ...
    struct tablegen_group_data {
        std::string inc;
        std::vector<std::string> opts;
    };
    using v_tablegen_group_data = std::vector<tablegen_group_data>;
    auto clang_tablegen_group = [&](auto &t, auto &&td, const v_tablegen_group_data &d) {
        for (auto &&p : d) {
            clang_tablegen(t, td, p.inc, p.opts, true);
        }
    };

    auto clang_diag_gen = [&](auto &t, const std::string &component) {
        clang_tablegen_group(t, "include/clang/Basic/Diagnostic.td", v_tablegen_group_data{
            {"clang/Basic/Diagnostic" + component + "Kinds.inc",{"-gen-clang-diags-defs","-clang-component=" + component}},
            {"clang/Basic/Diagnostic" + component + "Enums.inc",{"-gen-clang-diags-enums","-clang-component=" + component}},
            {"clang/Basic/Diagnostic" + component + "CompatIDs.inc",{"-gen-clang-diags-compat-ids","-clang-component=" + component}},
            {"clang/Basic/Diagnostic" + component + "Interface.inc",{"-gen-clang-diags-iface","-clang-component=" + component}},
        });
    };
    ...
    auto &Basic = clang.template addTarget<StaticLibraryTarget>("Basic");
    {
        ...
        Basic.Variables["MAX_CLANG_ABI_COMPAT_VERSION"] = Basic.Variables["PACKAGE_VERSION_MAJOR"];
        ...
        for (auto &c : {"Analysis", "AST", "Comment", "Common", "CrossTU",
                        "Driver", "Frontend", "InstallAPI", "Lex", "Parse", "Refactoring",
                        "Sema", "Serialization", "Trap"}) {
            clang_diag_gen(Basic, c);
        }

        clang_tablegen_group(Basic, "include/clang/Basic/Diagnostic.td", v_tablegen_group_data{
            {"clang/Basic/DiagnosticGroups.inc", { "-gen-clang-diag-groups" }},
            {"clang/Basic/DiagnosticIndexName.inc", { "-gen-clang-diags-index-name" }},
            {"clang/Basic/DiagnosticAllCompatIDs.inc", { "-gen-clang-diags-compat-ids" }},
        });
        clang_tablegen_group(Basic, "include/clang/Basic/Attr.td", v_tablegen_group_data{
            {"clang/Basic/AttrList.inc", { "-gen-clang-attr-list" }},
            {"clang/Basic/AttrParsedAttrList.inc", { "-gen-clang-attr-parsed-attr-list" }},
            ...
            {"clang/Basic/RegularKeywordAttrInfo.inc", { "-gen-clang-regular-keyword-attr-info" }},
            {"clang/Basic/CXX11AttributeInfo.inc", { "-gen-cxx11-attribute-info" }},
            {"clang/Basic/AttributeSpellingList.inc", { "-gen-attribute-spelling-list" }},
        });

        auto builtins = [&](const std::string &name, const std::string &suffix = "s") {
            clang_tablegen(Basic, "include/clang/Basic/" + name + ".td",
                "clang/Basic/" + name + ".inc", { "-gen-clang-builtin"s + suffix }, true);
        };
        builtins("Builtins");
        builtins("BuiltinsBPF");
        builtins("BuiltinsDirectX");
        builtins("BuiltinsHexagon");
        builtins("BuiltinsNVPTX");
        builtins("BuiltinsRISCV");
        builtins("BuiltinsSPIRVCommon");
        builtins("BuiltinsSPIRVVK");
        builtins("BuiltinsSPIRVCL");
        builtins("BuiltinsX86");
        builtins("BuiltinsX86_64");
        builtins("BuiltinTemplates", "-templates");
        ...
        clang_tablegen_group(Basic, "include/clang/Basic/arm_mve.td", v_tablegen_group_data{
        ...
        clang_tablegen_group(Basic, "include/clang/Basic/arm_sme.td", v_tablegen_group_data{
            {"clang/Basic/arm_sme_builtins.inc", { "-gen-arm-sme-builtins" }},
            {"clang/Basic/arm_sme_builtin_cg.inc", { "-gen-arm-sme-builtin-codegen" }},
            {"clang/Basic/arm_sme_sema_rangechecks.inc", { "-gen-arm-sme-sema-rangechecks" }},
            {"clang/Basic/arm_sme_streaming_attrs.inc", { "-gen-arm-sme-streaming-attrs" }},
            {"clang/Basic/arm_sme_builtins_za_state.inc", { "-gen-arm-sme-builtin-za-state" }},
        });
        clang_tablegen_group(Basic, "include/clang/Basic/arm_sve.td", v_tablegen_group_data{
            {"clang/Basic/arm_immcheck_types.inc",{"-gen-arm-immcheck-types"}},
            {"clang/Basic/arm_sve_builtins.inc",{"-gen-arm-sve-builtins"}},
            {"clang/Basic/arm_sve_builtins.json",{"-gen-arm-sve-builtins-json"}},
            ...
            {"clang/Basic/arm_sve_streaming_attrs.inc",{"-gen-arm-sve-streaming-attrs"}},
        });
        clang_tablegen_group(Basic, "include/clang/Basic/arm_cde.td", v_tablegen_group_data{
        ...
        // riscv
        clang_tablegen_group(Basic, "include/clang/Basic/riscv_vector.td", v_tablegen_group_data{
            ...
            {"clang/Basic/riscv_vector_builtin_sema.inc", { "-gen-riscv-vector-builtin-sema" }},
        });
        clang_tablegen_group(Basic, "include/clang/Basic/riscv_sifive_vector.td", v_tablegen_group_data{
            {"clang/Basic/riscv_sifive_vector_builtins.inc", { "-gen-riscv-sifive-vector-builtins" }},
            {"clang/Basic/riscv_sifive_vector_builtin_cg.inc", { "-gen-riscv-sifive-vector-builtin-codegen" }},
            {"clang/Basic/riscv_sifive_vector_builtin_sema.inc", { "-gen-riscv-sifive-vector-builtin-sema" }},
        });
        clang_tablegen_group(Basic, "include/clang/Basic/riscv_andes_vector.td", v_tablegen_group_data{
            {"clang/Basic/riscv_andes_vector_builtins.inc",{"-gen-riscv-andes-vector-builtins"}},
            {"clang/Basic/riscv_andes_vector_builtin_cg.inc",{"-gen-riscv-andes-vector-builtin-codegen"}},
            {"clang/Basic/riscv_andes_vector_builtin_sema.inc",{"-gen-riscv-andes-vector-builtin-sema"}},
        });
...

        {
            clang_tablegen_group(Basic, "include/clang/Basic/Attr.td", v_tablegen_group_data{
                ...
                // ?
                {"clang/AST/AttrDocTable.inc", { "-gen-clang-attr-doc-table" }},
                {"AttrDocTable.inc", { "-gen-clang-attr-doc-table" }},
            });
        ...
        clang_tablegen_group(Basic, "include/clang/Basic/Attr.td", v_tablegen_group_data{
            ...
            {"clang/Sema/AttrIsTypeDependent.inc", { "-gen-clang-attr-is-type-dependent" }},
        ...
        targets.tablegen(UTablegen, Basic, "include/clang/Options/Options.td",
            "clang/Options/Options.inc", { "-gen-opt-parser-defs" }, true,
    ...
    clang_tablegen(AST, "lib/AST/ByteCode/Opcodes.td", "Opcodes.inc", { "-gen-clang-opcodes" }, true);
    ...
    Sema += Sema.constructThisPackageDependency("llvm_project.llvm.Frontend.OpenACC.Gen");
...
{
    project<sw::ProjectTarget> llvm_project{s.addProject("llvm_project", "22.1.0")};
