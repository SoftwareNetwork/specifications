...
{
    auto &p = s.addProject("StellarGroup.hpx", "1.4.0");
    p += Git("https://github.com/STEllAR-GROUP/hpx", "{v}");
...
#define ADD_LIBRARY_WITH_CONFIG(x) ADD_LIBRARY(x); x.Public += config
    ...
    ADD_LIBRARY(logging);
    logging.Public += assertion, filesystem, timing;
...
#include <hpx/errors/throw_exception.hpp>
    ...
    ADD_LIBRARY(basic_execution);
    basic_execution.Public += errors;

    ADD_LIBRARY(concurrency);
    concurrency.Public += basic_execution;

    ADD_LIBRARY(topology);
    topology.Public += concurrency;
