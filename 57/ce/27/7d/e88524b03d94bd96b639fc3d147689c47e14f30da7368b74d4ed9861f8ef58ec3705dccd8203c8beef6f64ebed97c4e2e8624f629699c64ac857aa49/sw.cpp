...
{
    auto &p = s.addProject("graphicsmagick", "1.3.40");
    p += RemoteFile("https://sourceforge.net/projects/graphicsmagick/files/graphicsmagick/{v}/GraphicsMagick-{v}.tar.gz");

    auto &magick = p.addTarget<LibraryTarget>("magick");
    {
        if (magick.getBuildSettings().TargetOS.Type != OSType::Windows)
            magick.ExportAllSymbols = true;
        ...
        magick += "HasJBIG"_def;
        magick.Public += "org.sw.demo.mgk25.jbig.jbig"_dep;
