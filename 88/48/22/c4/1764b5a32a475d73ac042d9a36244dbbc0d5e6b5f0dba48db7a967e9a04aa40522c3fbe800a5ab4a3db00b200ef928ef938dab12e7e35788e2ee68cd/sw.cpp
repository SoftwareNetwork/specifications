...
{
    auto &sql = s.addProject("sqlcipher", "4.13.0");
    ...
    auto outdir = sqlcipher.BinaryDir;
    auto sqlite3h = outdir / "sqlite3.h";
    {
        ...
        sqlcipher.Private += "SQLITE_EXTRA_INIT=sqlcipher_extra_init"_d;
        sqlcipher.Private += "SQLITE_EXTRA_SHUTDOWN=sqlcipher_extra_shutdown"_d;
        sqlcipher.Private += "SQLITE_TEMP_STORE=2"_d;
        //sqlcipher.Private += "SQLITE_USER_AUTHENTICATION"_d;
        sqlcipher.Private += "SQLITE_OMIT_COMPILEOPTION_DIAGS"_d;
        ...
        sqlcipher.patch("ext/fts5/fts5_main.c", "static void sqlite3Fts5SetLocale", "void  sqlite3Fts5SetLocale");
        ...
        auto parse2y = outdir / "parse2.y";
        auto parse2 = outdir / "parse2.c";
        auto parse2h = outdir / "parse2.h";
        auto parse = outdir / "parse.c";
        auto parseh = outdir / "parse.h";
        auto opcodesh = outdir / "opcodes.h";
        auto pragmah = outdir / "pragma.h";
        auto opcodes = outdir / "opcodes.c";
        auto tmp1 = outdir / "tmp1";
        auto keywordhashh = outdir / "keywordhash.h";
        auto fts5parse2y = outdir / "fts5parse2.y";
        auto fts5parse2 = outdir / "fts5parse2.c";
        auto fts5parse2h = outdir / "fts5parse2.h";
        auto fts5parseh = outdir / "fts5parse.h";
        ...
        sqlcipher.addCommand(SW_VISIBLE_FUNCTION(merge_files))
            << cmd::in(parseh)
            << cmd::in("src/vdbe.c")
            << cmd::out(tmp1)
            ;
        sqlcipher.addCommand()
            << cmd::prog(mkkeywordhash)
        ...
        sqlcipher.addCommand()
            << cmd::prog("org.sw.demo.tcl.sh"_dep)
        ...
        sqlcipher.addCommand()
            << cmd::prog("org.sw.demo.tcl.sh"_dep)
            << cmd::wdir(pragmah.parent_path())
            << cmd::in("tool/mkpragmatab.tcl")
            << cmd::end()
            << cmd::out(pragmah)
            ;
        sqlcipher.addCommand()
        ...
        sqlcipher.pushFrontToFileOnce("src/sqlcipher.c", "#include <stdint.h>");
    ...
    auto &shell = sql.addTarget<ExecutableTarget>("shell");
    {
        ...
        //auto sqlcipher_bpdir = to_printable_string(normalize_path(sqlcipher.BinaryDir));

        {
            ...
            c << cmd::prog("org.sw.demo.tcl.sh"_dep)
                ...
                << cmd::in("ext/intck/sqlite3intck.c", cmd::Skip)
                << cmd::in("ext/intck/sqlite3intck.h", cmd::Skip)
                << cmd::in("ext/misc/appendvfs.c", cmd::Skip)
                << cmd::in("ext/misc/base64.c", cmd::Skip)
                << cmd::in("ext/misc/base85.c", cmd::Skip)
                ...
                << cmd::in("ext/misc/pcachetrace.c", cmd::Skip)
                << cmd::in("ext/misc/regexp.c", cmd::Skip)
                << cmd::in("ext/misc/series.c", cmd::Skip)
                << cmd::in("ext/misc/sha1.c", cmd::Skip)
                << cmd::in("ext/misc/shathree.c", cmd::Skip)
                << cmd::in("ext/misc/sqlar.c", cmd::Skip)
                << cmd::in("ext/misc/uint.c", cmd::Skip)
                << cmd::in("ext/misc/vfstrace.c", cmd::Skip)
                << cmd::in("ext/misc/windirent.h", cmd::Skip)
                << cmd::in("ext/misc/zipfile.c", cmd::Skip)
                << cmd::in("ext/recover/dbdata.c", cmd::Skip)
                << cmd::in("ext/recover/sqlite3recover.c", cmd::Skip)
                << cmd::in("ext/recover/sqlite3recover.h", cmd::Skip)
                ;
        }
        {
            //shell.patch("tool/mksqlite3c.tcl", "tsrc/sqlite3.h", sqlcipher_bdir + "/sqlite3.h");
            //shell.patch("tool/mksqlite3c.tcl", "parse.c", "parse2.c");
            // todo: replace other tsrc->...
            // or find another way
            // because tsrc/sqlite3.h is met for the second time

            // take all files from tool/mksqlite3c.tcl and copy to bdir?
            // TODO: we have some missing files like fts5.c etc. before we do this

            auto c = shell.addCommand();
            c << cmd::prog("org.sw.demo.tcl.sh"_dep)
                << cmd::wdir(shell.SourceDir)
                << cmd::in("tool/mksqlite3c.tcl")
                << "--srcdir" << sqlcipher_bdir
                << cmd::end()
                << cmd::out("sqlite3.c")
                << cmd::in(sqlite3h)
                ;
