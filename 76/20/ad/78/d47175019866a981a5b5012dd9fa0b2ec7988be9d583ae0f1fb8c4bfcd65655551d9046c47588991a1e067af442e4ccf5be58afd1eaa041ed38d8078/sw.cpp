...
{
    auto &opencv = s.addProject("intel.opencv", "5.0.0");
    ...
    auto &core = add_target("core");
    {
        ...
        add_dispatch_file(core, "minmax");
        add_dispatch_file(core, "transpose");
        add_dispatch_file(core, "undistort");
        add_dispatch_file(core, "reduce");
        add_dispatch_file(core, "nan_mask");
        add_dispatch_file(core, "warp_kernels");
    ...
    auto &geometry = add_target_with_core("geometry");
    geometry.Public += flann;
    ...
    imgproc.Public += geometry;

    embed2("pub.egorpugin.primitives.tools.embedder2" PRIMITIVES_VERSION ""_dep, imgproc, "modules/imgproc/fonts/Rubik.ttf.gz", "builtin_font_sans1.h");
    imgproc.writeFileOnce("builtin_font_sans.h", R"(
static const unsigned char OcvBuiltinFontSans[] = {
#include "builtin_font_sans1.h"
};
)");

    embed2("pub.egorpugin.primitives.tools.embedder2" PRIMITIVES_VERSION ""_dep, imgproc, "modules/imgproc/fonts/Rubik-Italic.ttf.gz", "builtin_font_italic1.h");
    imgproc.writeFileOnce("builtin_font_italic.h", R"(
static const unsigned char OcvBuiltinFontItalic[] = {
#include "builtin_font_italic1.h"
};
)");

    auto &features = add_target("features");
    features.Public += imgproc;
    features.Public += "HAVE_OPENCV_FLANN"_def;
    add_dispatch_file(features, "sift");
    ...
    auto &calib = add_target("calib");
    calib.Public += features;
    add_dispatch_file(calib, "undistort");

    auto &photo = add_target("photo");
    photo.Public += imgproc;

    auto &video = add_target("video");
    add_dispatch_file(video, "lkpyramid");
    ...
    auto &dnn = add_target("dnn");
    {
        ...
        dnn -= "modules/dnn/src/caffe/.*"_rr;
        dnn -= "modules/dnn/src/tensorflow/.*"_rr;
        ...
        add_dispatch_file(dnn, "layers/cpu_kernels/nary_eltwise_kernels");
        add_dispatch_file(dnn, "layers/cpu_kernels/activation_kernels");
        add_dispatch_file(dnn, "layers/cpu_kernels/gridsample_kernels");
        add_dispatch_file(dnn, "layers/cpu_kernels/transpose_kernels");
        add_dispatch_file(dnn, "layers/cpu_kernels/reduce2_kernels");
        add_dispatch_file(dnn, "layers/cpu_kernels/conv2_depthwise");
        add_dispatch_file(dnn, "layers/cpu_kernels/conv2_kernels");
        add_dispatch_file(dnn, "layers/cpu_kernels/fast_gemm_kernels");
        add_dispatch_file(dnn, "int8layers/layers_common");
        add_dispatch_file(dnn, "int8layers/conv2_int8_kernels");
    ...
    objdetect.Public += videoio, calib, dnn;
    ...
    stitching.Public += objdetect, calib, features;
    stitching += "HAVE_OPENCV_Xfeatures"_def;

    auto &stereo = add_target("stereo");
    stereo.Public += imgproc; //calib, videoio, video, highgui;

    auto &ptcloud = add_target("ptcloud");
    ptcloud.Public += imgproc;
    ...
    //aruco.Public += calib, objdetect;

    auto &bgsegm = add_target("bgsegm");
    bgsegm.Public += calib, video;
    ...
    line_descriptor.Public += features, videoio, highgui;
    ...
    {
        xfeatures2d.Public += features;
        xfeatures2d += "OPENCV_Xfeatures_HAS_VGG_DATA"_def;
    ...
    ccalib.Public += calib, highgui, xfeatures2d, stereo;

    auto &ximgproc = add_target("ximgproc");
    ximgproc.Public += calib, video, videoio, highgui, stereo;
    ...
    face.Public += imgproc, objdetect, xobjdetect;
    ...
    structured_light.Public += phase_unwrapping, calib;
