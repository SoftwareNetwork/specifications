...
{
    auto &p = s.addProject("gnu.bison", "3.5.0");
    p += RemoteFile("https://ftp.gnu.org/gnu/bison/bison-{M}.{m}{po}.tar.xz");

    auto &gnulib = p.addTarget<StaticLibraryTarget>("gnulib");
    {
...
char * strsignal(int);
    ...
    auto &bison = p.addTarget<ExecutableTarget>("bison");
    {
        ...
        bison.Private += "PKGDATADIR=\".\""_d;
        bison.Public += gnulib;

        if (bison.getBuildSettings().TargetOS.Type == OSType::Windows)
        {
