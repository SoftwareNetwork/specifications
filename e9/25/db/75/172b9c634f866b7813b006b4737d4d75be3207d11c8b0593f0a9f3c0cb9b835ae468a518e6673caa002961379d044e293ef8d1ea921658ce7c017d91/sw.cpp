...
{
    auto &freetype = s.addTarget<LibraryTarget>("freetype", "2.9.1");
    std::string patch;
    if (freetype.getPackage().version.getPatch())
        patch = "-" + std::to_string(freetype.getPackage().version.getPatch());
    freetype += RemoteFile("https://git.savannah.gnu.org/cgit/freetype/freetype2.git/snapshot/VER-{M}-{m}" + patch + ".tar.gz");
