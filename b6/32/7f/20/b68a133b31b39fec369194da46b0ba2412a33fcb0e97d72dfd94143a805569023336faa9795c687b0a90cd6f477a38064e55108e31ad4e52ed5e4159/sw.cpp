...
T &add_subproject(Target &base, const String &name)
{
    ...
    p += Git("https://github.com/qt/" + qt_name, "v{M}.{m}.{p}-lts-lgpl");
...
{
    auto &qt = s.addProject("qtproject.qt", "5.15.12");
    ...
    auto write_tracepoints = [&tracegen](auto &t)
    {
        ...
        c
            ...
            ;*/
        t.writeFileOnce("qt" + t.getPackage().getPath().back() + "_tracepoints_p.h", "#include <private/qtrace_p.h>");
    };
    ...
    // base
    {
        ...
        // core
        {
            //core ^= "src/3rdparty/harfbuzz/src/.gitignore";
            //core ^= "src/corelib/serialization/.gitignore";
            core -=
                "mkspecs/.*\\.h"_rr;
            core +=
            ...
            auto sqt = syncqt("pub.egorpugin.primitives.tools.syncqt"_dep, core, { "QtCore", "QtPlatformHeaders" });
        ...
        // gui
        {
            ...
            gui.replaceInFileOnce("text/qtextmarkdownimporter.cpp", "#include \"../../3rdparty/md4c/md4c.h\"", "#include <md4c.h>");
...
Q_IMPORT_PLUGIN()" + name + R"();
    ...
    NativeExecutedTarget *qml_imports_layouts, *qml_imports_window, *pquick;
    {
        ...
        // qml
        {
            ...
            //qml.Protected += "qml"_idir;

            if (!qml.DryRun)
            {
                auto dir2 = qml.BinaryDir / "private";
                auto dummy = dir2 / "sw_copy_headers3.txt";
                if (!fs::exists(dummy))
                {
                    fs::create_directories(dir2);
                    for (auto &&d : {".", "compiler", "jsruntime", "qml"}) {
                        for (auto &f : fs::directory_iterator(qml.SourceDir / d)) {
    ...
    // winextras
    {
        ...
        syncqt("pub.egorpugin.primitives.tools.syncqt"_dep, winextras, { "QtWinExtras" });
        auto &t = winextras;
