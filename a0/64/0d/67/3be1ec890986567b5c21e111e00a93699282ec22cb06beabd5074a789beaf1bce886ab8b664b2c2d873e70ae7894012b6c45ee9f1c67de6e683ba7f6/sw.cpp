...
{
    auto &cpprestsdk = s.addTarget<LibraryTarget>("microsoft.cpprestsdk", "2.10.17");
    cpprestsdk += Git("https://github.com/Microsoft/cpprestsdk", "{v}");
    ...
    if (cpprestsdk.getBuildSettings().TargetOS.Type == OSType::Windows)
    {
    ...
    if (cpprestsdk.getBuildSettings().TargetOS.Type == OSType::Windows)
    {
        cpprestsdk += "UNICODE"_def;
        cpprestsdk.Public += "bcrypt.lib"_slib;
        cpprestsdk.Public += "httpapi.lib"_slib;
        cpprestsdk.Public += "winhttp.lib"_slib;
    }
