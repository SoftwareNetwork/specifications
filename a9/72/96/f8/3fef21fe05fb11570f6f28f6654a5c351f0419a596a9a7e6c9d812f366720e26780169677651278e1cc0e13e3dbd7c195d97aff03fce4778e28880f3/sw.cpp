auto get_source(const String &component)
{
...
#undef HANDLE_EXTENSION)");
    ...
    //
    auto &TextAPI = llvm.addTarget<StaticLibraryTarget>("TextAPI");
    ...
    auto &intrinsics_gen = llvm.addTarget<StaticLibraryTarget>("intrinsics_gen");
    {
    ...
    //
    auto &BitstreamReader = llvm.addTarget<StaticLibraryTarget>("Bitstream.Reader");
    BitstreamReader += "lib/Bitstream/Reader/.*"_rr;
    BitstreamReader.Public += Support;

    //
    auto &Remarks = llvm.addTarget<StaticLibraryTarget>("Remarks");
    ...
    Analysis -= "lib/Analysis/ReleaseModeModelRunner.cpp";
    ...
    //
    auto &FrontendOpenMP = llvm.addTarget<StaticLibraryTarget>("Frontend.OpenMP");
    FrontendOpenMP += "lib/Frontend/OpenMP/.*"_rr;
    FrontendOpenMP.Public += TransformUtils;
    tablegen(FrontendOpenMP, "include/llvm/Frontend/OpenMP/OMP.td", "llvm/Frontend/OpenMP/OMP.inc", { "-gen-directive-gen" });
    ...
    //
    auto &AggressiveInstCombine = llvm.addTarget<StaticLibraryTarget>("AggressiveInstCombine");
    ...
    //
    auto &CFGuard = llvm.addTarget<StaticLibraryTarget>("CFGuard");
    ...
    //
    auto &HelloWorldPass = llvm.addTarget<StaticLibraryTarget>("HelloNew");
    HelloWorldPass += "lib/Transforms/HelloNew/.*"_rr;
    ...
    Passes.Public += AggressiveInstCombine, Analysis, CodeGen, HelloWorldPass, InstCombine, Instrumentation, Vectorize, ipo;
    ...
    auto add_arch = [&](const std::string &dir)
    {
        ...
        auto &ATablegen = d.addTarget<StaticLibraryTarget>("CommonTableGen");
        if (!ATablegen.DryRun)
        {
        ...
        //
        auto &AUtils = d.addTarget<StaticLibraryTarget>("Utils");
        ...
        //
        auto &AAsmPrinter = d.addTarget<StaticLibraryTarget>("AsmPrinter");
        ...
        //
        auto &ADisassembler = d.addTarget<StaticLibraryTarget>("Disassembler");
...
void build_clang(ProjectTarget &llvm_project, LlvmTargets &targets)
{
    ...
    auto &ToolingInclusions = clang.addTarget<StaticLibraryTarget>("Tooling.Inclusions");
    ...
    for (auto d : { "Analysis", "BitReader", "BitWriter", "Core",
        "Coroutines", "Coverage", "Frontend.OpenMP", "ipo", "IRReader", "InstCombine", "Instrumentation", "LTO",
...
void build_libs(ProjectTarget &llvm_project)
{
    auto is_musl = [](auto &t)
    {
        return
...
{
    auto &llvm_project = s.addProject("llvm_project", "12.0.0");
    LlvmTargets targets;
