...
{
    auto &p = s.addProject("wxWidgets", "3.1.2");
    ...
    {
        static const auto files_cmake_fn = "build/cmake/files.cmake";
        static const auto files = read_file(t.SourceDir / files_cmake_fn);

        t += files_cmake_fn;

        auto read_files = [&t](const String &n, const path &base = {}, bool is_header = false)
        {
            String f = "set(" + n;
            auto p = files.find(f);
            if (p == files.npos)
    ...
    auto &base = add_target("base");
    {
        ...
        else if (s.Settings.TargetOS.Type == OSType::IOS)
        ...
        auto add_basic_settings = [&s](auto &base)
        {
            ...
            base.Variables["wxUSE_UNIX"] = int(s.Settings.TargetOS.Type != OSType::Windows);
            ...
            if (s.Settings.TargetOS.Type == OSType::Windows || s.Settings.TargetOS.Type == OSType::Macos)
    ...
    auto &net = add_target("net");
    {
        ...
        else if (s.Settings.TargetOS.Type == OSType::Macos) /// apple
