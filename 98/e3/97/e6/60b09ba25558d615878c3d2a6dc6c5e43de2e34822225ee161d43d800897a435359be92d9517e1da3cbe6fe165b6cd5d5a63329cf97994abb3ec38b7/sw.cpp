...
static Files automoc(const DependencyPtr &moc, NativeExecutedTarget &t, const std::vector<moc_file> &additions = {})
{
    ...
    for (auto &f : files)
    {
        ...
        c->push_back("-o");
        c->push_back(o);
        c->push_back("--output-json");
        c->addInput(f.p);
        c->addOutput(o);
        c->addOutput(path{o} += ".json");
    ...
    c
        << cmd::prog(moc)
        << "-o"
        << cmd::out("metatypes.json")
        << "--collect-json"
        ;
    t -= "metatypes.json";
    for (auto &f : mocs) {
        c << cmd::in(path{f} += ".json");
    }
    // also there is some manual moc files processing
    // e.g. for moc_qobject.cpp
    /*
    -o src/corelib/kernel/moc_qobject.cpp
    kernel/qobject.h
    --output-json
    -DWIN32
...
struct QtLibrary
{
    ...
    void print(NativeExecutedTarget &t)
    {
        ...
        t.Protected += IncludeDirectory(t.BinaryDir / actual_name / "private");
...
void build(Solution &s)
{
    ...
    //
    auto &qt = s.addProject("qtproject.qt", "6.11.0");
    ...
    auto &syncqt_target = base_tools.addTarget<ExecutableTarget>("syncqt");
    ...
    qt_desc.config.public_.definitions["QT_COPYRIGHT"] = "\"\"";
    qt_desc.config.public_.definitions["QT_COPYRIGHT_YEAR"] = "\"\"";
    ...
    auto write_tracepoints = [&tracegen, &tracepointgen](auto &t, auto && ... args)
    {
        ...
        /*{
            ...
            c << providername;
            c << cmd::out(fn);
            // worked without includes
            auto idirs = find_idirs(t);
            for (auto &d : idirs)
                c << ("I" + to_string(normalize_path(d)));
            //
            ((c << cmd::in(args)),...);
        }*/
    ...
    auto exports_setup = [&](auto &t, const String &custom_name = {}, bool bootstrap = false) {
        ...
        if (custom_name == "qmlworkerscript")
            sentence = "QmlWorkerScript";
        if (custom_name == "qmlmeta")
            sentence = "QmlMeta";
...
Q_IMPORT_PLUGIN()" + name + R"();
    ...
    // base
    {
        ...
        auto &bootstrap = base_tools.addTarget<StaticLibraryTarget>("bootstrap");
        {
            ...
            qt_desc2.config.public_.features.insert({"openssl_hash", false});
            qt_desc2.config.public_.features.insert({"dom", true});
            qt_desc2.config.public_.features.insert({"dom", true});
            qt_desc2.print(bootstrap);
            ...
            if (bootstrap.getBuildSettings().TargetOS.Type == OSType::Windows) {
                bootstrap.Variables["QT_DISABLE_DEPRECATED_UP_TO"] = "0x040800";
            } else {
                bootstrap.Variables["QT_DISABLE_DEPRECATED_UP_TO"] = "0x050000";
            }
            bootstrap.Variables["QT_WARN_DEPRECATED_UP_TO"] = "0x070000";
            bootstrap.configureFile("src/corelib/global/qtdeprecationdefinitions.h.in", "QtCore/qtdeprecationdefinitions.h");
            /*bootstrap.patch("src/corelib/global/qconfig-bootstrapped.h"
        ...
        // core
        {
            ...
            core.Public += "org.sw.demo.openssl.ssl"_dep;
            // natvis
            core.Public += "org.sw.demo.qtproject.qt.labs.vstools.natvis.qt6"_dep;

            qt_core_desc.config.public_.features.insert({ "debug", core.getBuildSettings().Native.ConfigurationType == ConfigurationType::Debug });
            if (core.getBuildSettings().TargetOS.Type == OSType::Windows)
            {
                ...
                qt_core_desc.config.public_.features.insert({ "cxx23_stacktrace", false });
                qt_core_desc.config.public_.features.insert({ "memmem", false });
                qt_core_desc.config.public_.features.insert({ "memrchr", false });
                ...
                qt_core_desc.config.public_.features.insert({ "icu", false });

                qt_core_desc.config.public_.features.insert({ "posix_shm", false });
                qt_core_desc.config.public_.features.insert({ "posix_sem", false });
                qt_core_desc.config.public_.features.insert({ "sysv_shm", false });
                qt_core_desc.config.public_.features.insert({ "sysv_sem", false });

                qt_core_desc.config.public_.features.insert({ "windows_ioring", true });
            }
            else
            {
                ...
                qt_core_desc.config.public_.features.insert({ "posix_shm", true });
                qt_core_desc.config.public_.features.insert({ "posix_sem", true });
                qt_core_desc.config.public_.features.insert({ "sysv_shm", true });
                qt_core_desc.config.public_.features.insert({ "sysv_sem", true });
            }
            ...
            qt_desc.config.public_.features.insert({"datetimeparser", true});
            qt_desc.print(core);

            qt_core_desc.config.public_.features.insert({"datetimeparser", true});
            qt_core_desc.print(core);

            if (core.getBuildSettings().TargetOS.Type == OSType::Windows) {
                core.Variables["QT_DISABLE_DEPRECATED_UP_TO"] = "0x040800";
            } else {
                core.Variables["QT_DISABLE_DEPRECATED_UP_TO"] = "0x050000";
            }
            // 6.11 has bug in src/gui/painting/qpainter.h
            // they removed actual impl in qpainter.cpp file
            /*
                QT_GUI_INLINE_SINCE(6, 11)
                void setBrush(const QBrush &brush);
            */
            core.Variables["QT_DISABLE_DEPRECATED_UP_TO"] = "QT_VERSION_CHECK(6,11,0)";
            core.Variables["QT_WARN_DEPRECATED_UP_TO"] = "0x070000";
            core.configureFile("global/qtdeprecationdefinitions.h.in", "QtCore/qtdeprecationdefinitions.h");
...
#define QT_CONFIGURE_PREFIX_PATH qt_configure_prefix_path_str + 12
            ...
            // since what sdk? Windows 21H2 (build 22000)
            core -= "io/qioring_win.cpp";
            core -= "io/qrandomaccessasyncfile_qioring.cpp";
            if (core.getBuildSettings().TargetOS.Type == OSType::Windows) {
                //if (auto v = core.getBuildSettings().TargetOS.Version; v > Version{10,0,22000}) {
                    core += "QT_RANDOMACCESSASYNCFILE_QIORING"_def;
                    core += "io/qioring_win.cpp";
                    core += "io/qrandomaccessasyncfile_qioring.cpp";
                //}
            }
...

            {
                ...
                c << cmd::in("mimetypes/3rdparty/tika-mimetypes.xml");
            ...
            // win/msvc only?
            //core.patch("plugin/qlibrary_p.h", "Q_DECLARE_LOGGING_CATEGORY(", "//Q_DECLARE_LOGGING_CATEGORY (");
        }
  ...
  //     "(^|/)qrhi\\.h$|(^|/)qrhi_platform\\.h$|(^|/)qshader\\.h$|(^|/)qshaderdescription\\.h$"
            ...
            if (!gui.DryRun)
            {
                auto dir = gui.BinaryDir / "symlink";
                if (!fs::exists(dir))
                {
                    fs::create_directories(dir);
                    fs::create_directory_symlink(gui.SourceDir, dir / "QtGui");
                }
                gui.Public += IncludeDirectory{dir};
            }

            gui.Public += "org.sw.demo.glennrp.png"_dep;
            gui += "org.sw.demo.mity.md4c"_dep;
            gui += "org.sw.demo.baldurk.renderdoc"_dep;
            gui.Public += core;

            gui.Public -= dbus;
            gui -= "org.sw.demo.gpuopen.D3D12MemoryAllocator"_dep;
            qt_gui_desc.config.public_.features.insert({ "vxworksevdev", false });
            qt_gui_desc.config.public_.features.insert({ "test_gui", false });
            if (gui.getBuildSettings().TargetOS.Type == OSType::Windows)
            {
                gui += "org.sw.demo.gpuopen.D3D12MemoryAllocator"_dep;
                ...
                qt_gui_desc.config.public_.features.insert({ "wayland", false });
            ...
            if (gui.getBuildSettings().TargetOS.Type == OSType::Linux)
            {
                ...
                qt_gui_desc.config.public_.features.insert({ "wayland", true });
            ...
            if (gui.getBuildSettings().TargetOS.isApple()) {
                qt_gui_desc.config.public_.features.insert({ "metal", true });
            } else {
                qt_gui_desc.config.public_.features.insert({ "metal", false });
            }
    ...
    "styles/images/warning-32.png",
        ...
        // network
        {
            ...
            {
                network -= "kernel/qnetworkproxy_darwin.cpp";
            }
        ...
        auto &modernwindows = styles.addTarget<LibraryTarget>("modernwindows");
        {
            common_setup(modernwindows);
            modernwindows.setOutputDir("plugins/styles");
            modernwindows +=
                "src/plugins/styles/modernwindows/.*"_rr;
            modernwindows.Public +=
                "src/plugins/styles/modernwindows"_id;

            if (modernwindows.getBuildSettings().TargetOS.Type == OSType::Windows)
            {
                modernwindows.Public += "gdi32.lib"_slib;
                modernwindows.Public += "user32.lib"_slib;
            }

            modernwindows.Public += widgets;

            automoc(moc, modernwindows);
            make_qt_plugin(modernwindows, "QModernWindowsStylePlugin");
        }
    ...
    // make builtin function! (??? maybe just configure?)
    // TODO: not finished
    auto make_qml_plugin = [](auto &&t, String name) {
        //if (t.DryRun) {
        //    return;
        //}
        //String text = read_file(t.SourceDir / "Qt6QmlPluginTemplate.cpp.in");
        ...
        String qt_qml_plugin_intro = "QT_DECLARE_EXTERN_SYMBOL_VOID(" + register_types_function_name + ")";
        ...
        //}
        auto set_var = [&](auto &&k, auto &&v) {
            //boost::replace_all(text, k, v);
            t.Variables[k] = v;
        };
        set_var("qt_qml_plugin_intro", qt_qml_plugin_intro);
        set_var("qt_qml_plugin_class_name", qt_qml_plugin_class_name);
        set_var("qt_qml_plugin_constructor_content", qt_qml_plugin_constructor_content);
        set_var("qt_qml_plugin_outro", qt_qml_plugin_outro);
        set_var("qt_qml_plugin_moc_include_name", qt_qml_plugin_moc_include_name);
        //t.writeFileOnce(generated_cpp_file_name_base + ".cpp", text);
        t.configureFile("Qt6QmlPluginTemplate.cpp.in", generated_cpp_file_name_base + ".cpp");
    ...
    NativeExecutedTarget *qml_imports_layouts, *qml_imports_window, *pquick;
    {
        ...
        struct qmltypes_desc {
            std::string import_name;
            int major{}, minor{}, past_major{};
            std::vector<Target*> deps;
        };
        auto generate_qmltypes = [&](auto &t, const qmltypes_desc &d) {
            auto c = t.addCommand();
            c
                << cmd::prog(qmltyperegistrar)
                << cmd::out("plugins.qmltypes", cmd::Prefix{"--generate-qmltypes="})
                << "--import-name=" + d.import_name
                << std::format("--major-version={}", d.major)
                << std::format("--minor-version={}", d.minor)
                ;
            if (d.past_major) {
                c
                    << "--past-major-version" << d.past_major
                    ;
            }
            c
            ...
            for (auto &&d : d.deps) {
                c
                    << cmd::in(d->BinaryDir / "metatypes.json", cmd::Prefix{"--foreign-types="}, cmd::Skip, cmd::DoNotAddToTargets)
                    ;
            }
            c
                << "--private-includes"
                << "-o"
                << cmd::out("qmltyperegistrations.cpp")
                << cmd::in("metatypes.json")
                ;
        };
        ...
        // qml
        {
            ...
            generate_qmltypes(t, {.import_name="QML",.major=1, .minor=0,.deps={&core,&network}});
        }
        // post qml
        {
        ...
        auto &qml_models = qml.addLibrary("models");
        {
            ...
            generate_qmltypes(t, {.import_name="QtQml.Models",.major=6,.minor=11,
                .deps={&core,&qml,&network}});
        ...
        auto &qml_workerscript = qml.addLibrary("workerscript");
        {
            ...
            common_setup(t, "qmlworkerscript");
            ...
            generate_qmltypes(t, {.import_name="QtQml.WorkerScript",.major=6,.minor=11,
                .deps={&core,&qml,&network}});
        ...
        auto &qml_meta = qml.addLibrary("meta");
        {
            auto &t = qml_meta;
            common_setup(t, "qmlmeta");

            String module = "QtQmlMeta";
            //auto sqt = syncqt("pub.egorpugin.primitives.tools.syncqt"_dep, t, { module });
            t.SourceDir /= "src/qmlmeta";
            qt_syncqt(t, module, ".");

            t += ".*"_r;
            //t += "QT_BUILD_QMLMETA_LIB"_def;

            t.Public += qml_models, qml_workerscript;

            auto mocs = automoc(moc, t);
            //SW_QT_ADD_MOC_DEPS(t);

            generate_qmltypes(t, {.import_name="QtQml",.major=6,.minor=11,.past_major=2,
                .deps={&core,&qml,&network,&qml_models,&qml_workerscript}});
        ...
        pquick = &quick;
        {
            ...
            quick.Public += qml, gui, qml_models, qml_meta, opengl;

            //make_qml_plugin(quick, "QtQuick");

            auto mocs = automoc(moc, t);
            generate_qmltypes(t, {.import_name="QtQuick",.major=6,.minor=11,
                .deps={&core,&gui,&qml,&network,&qml_models,&qml_meta,&qml_workerscript,&opengl}});
    ...
    auto &tools = add_subproject<Project>(qt, "tools");
    {
        ...
        auto &qmake_parser = linguist.addTarget<StaticLibraryTarget>("qmake_parser");
        {
            auto &t = qmake_parser;
            common_setup(t);
            t.SourceDir /= "src/linguist/shared/qmake-parser";
            t += ".*\\.h"_rr;
            t -= ".*\\.cpp"_rr;
            ...
            // QT_NO_CAST_TO_ASCII
            t += "QT_USE_QSTRINGBUILDER"_d;

            t.Public += "PROEVALUATOR_CUMULATIVE"_d;
            t.Public += "PROEVALUATOR_DEBUG"_d;
            t.Public += "PROEVALUATOR_INIT_PROPS"_d;
            t += "QMAKE_BUILTIN_PRFS"_d;
            t += "QMAKE_OVERRIDE_PRFS"_d;
            if (t.getBuildSettings().TargetOS.Type == OSType::Windows)
            {
        ...
        auto &trlib = linguist.addTarget<StaticLibraryTarget>("trlib");
        {
            auto &t = trlib;
            common_setup(t);
            t.SourceDir /= "src/linguist/shared";
            t += ".*\\.h"_rr;
            t -= ".*\\.cpp"_rr;
            // don't use .cpp regex, there will be ling.tools run failures
            t +=
                ...
                "trlib/java.cpp",
                "trlib/ui.cpp",
                "trlib/metastrings.cpp"//,
                //"trlib/qdeclarative.cpp" // or QT_NO_QML
                ;
            // QT_NO_CAST_FROM_ASCII
            // QT_NO_CAST_TO_ASCII
            t += "QT_USE_QSTRINGBUILDER"_d;
        ...
        auto &linguist_project = linguist.addTarget<StaticLibraryTarget>("linguist_project");
        {
            auto &t = linguist_project;
            common_setup(t);
            t.SourceDir /= "src/linguist/shared/linguistproject";
            t += ".*\\.h"_rr;
            // don't use .cpp regex, there will be ling.tools run failures
            t +=
        ...
        auto &release = linguist.addTarget<ExecutableTarget>("release");
        {
            auto &t = release;
            common_setup(t);
            t += "src/linguist/lrelease/main.cpp";
            t += "src/linguist/shared/numerus.cpp";
            t += "src/linguist/shared/po.cpp";
            t += "src/linguist/shared/qm.cpp";
            t += "src/linguist/shared/qph.cpp";
            t += "src/linguist/shared/validator.cpp";
            t += "src/linguist/shared/translator.cpp";
            t += "src/linguist/shared/translatormessage.cpp";
            t += "src/linguist/shared/ts.cpp";
            t += "src/linguist/shared/xliff.cpp";
            t += "src/linguist/shared/xmlparser.cpp";
            t += "src/linguist/shared/releasehelper.cpp";

            t += "LINGUIST_CONSOLE_APPLICATION"_d;
            if (t.getBuildSettings().TargetOS.Type == OSType::Windows)
            {
                t.Private += "UNICODE"_d;
            }

            t.Public += linguist_project;
            //t.Public += formats;
        }
        ...
        auto &update = linguist.addTarget<ExecutableTarget>("update");
        {
            auto &t = update;
            common_setup(t);
            t += "src/linguist/lupdate/main.cpp";
            t += "src/linguist/shared/po.cpp";
            t += "src/linguist/shared/qm.cpp";
            t += "src/linguist/shared/qph.cpp";
            t += "src/linguist/shared/ts.cpp";
            ...
            t.replaceInFileOnce("src/linguist/lupdate/main.cpp",
    ...
    auto &wayland = add_subproject<Project>(qt, "wayland");
    {
        ...
        auto &hwi_egl = wayland.addLibrary("hardwareintegration.client.wayland.egl");
        {
            ...
            //auto sqt = syncqt("pub.egorpugin.primitives.tools.syncqt"_dep, hwi_egl, { "" });
            qt_syncqt(t, "QtWaylandEglClientHwIntegration", "src/hardwareintegration/compositor/wayland-egl");
        ...
        auto &adwaita = wayland.addLibrary("plugins.decorations.adwaita");
        {
            ...
            adwaita += "src/plugins/decorations/adwaita/.*"_rr;
            automoc(moc, adwaita);
            adwaita.Public += client;
