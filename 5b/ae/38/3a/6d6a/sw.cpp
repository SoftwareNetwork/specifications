...
{
    auto &s = sln.addDirectory("demo.hdfgroup");
    auto &p = s.addProject("hdf5", "1.10.1");
    p.Source = RemoteFile("https://support.hdfgroup.org/ftp/HDF5/releases/hdf5-{M}.{m}/hdf5-{v}/src/hdf5-{v}.tar.bz2");

    auto &hdf5 = p.addTarget<LibraryTarget>("hdf5");
    ...
    if (s.Settings.TargetOS.Type == OSType::Windows)
    {
        ...
        if (s.Settings.Native.CompilerType == CompilerType::MSVC)
            hdf5.Variables["H5_HAVE_VISUAL_STUDIO"] = "1";
    }
    else
    ...
    auto &H5Tinit = p.addTarget<ExecutableTarget>("tools.H5Tinit");
    H5Tinit += "src/H5detect.c";
    H5Tinit += IncludeDirectory{ hdf5.BinaryDir.string() };

    {
        auto c = std::make_shared<Command>();
        c->setProgram(H5Tinit);
        c->redirectStdout(hdf5.BinaryDir / "H5Tinit.c");
        hdf5 += hdf5.BinaryDir / "H5Tinit.c";
    }
    (H5Tinit + hdf5)->IncludeDirectoriesOnly = true;
    (hdf5 + H5Tinit)->Dummy = true;

    auto &H5make_libsettings = p.addTarget<ExecutableTarget>("tools.H5make_libsettings");
    H5make_libsettings += "src/H5make_libsettings.c";
    H5make_libsettings += IncludeDirectory{ hdf5.BinaryDir.string() };

    {
        auto c = std::make_shared<Command>();
        c->setProgram(H5make_libsettings);
        c->redirectStdout(hdf5.BinaryDir / "H5lib_settings.c");
        hdf5 += hdf5.BinaryDir / "H5lib_settings.c";
    }
    (H5make_libsettings + hdf5)->IncludeDirectoriesOnly = true;
    (hdf5 + H5make_libsettings)->Dummy = true;

    //
    auto &hdf5cpp = p.addTarget<LibraryTarget>("hdf5cpp");
    ...
    //
    auto &hdf5hl = p.addTarget<LibraryTarget>("hdf5hl");
    ...
    //
    auto &hdf5hlcpp = p.addTarget<LibraryTarget>("hdf5hlcpp");
...
void check(Checker &c)
{
    ...
    s.checkDeclarationExists("O_DIRECT");

    for (auto &[d, c] : s.checks)
        c->Prefixes.insert("H5_");
}
