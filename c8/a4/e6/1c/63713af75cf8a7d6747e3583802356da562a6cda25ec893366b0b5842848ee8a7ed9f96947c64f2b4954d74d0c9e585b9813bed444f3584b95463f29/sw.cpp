...
{
    auto &p = s.addProject("pcre", "10.47.0");
    ...
    auto add_pcre = [&p](int bits) -> decltype(auto)
    {
        ...
        pcre.Variables["PCRE2_MAX_VARLOOKBEHIND"] = 256;
        ...
        pcre.configureFile("src/config-cmake.h.in", pcre.BinaryPrivateDir / "config.h");
    ...
    pcreposix += "PCRE2POSIX_EXP_DECL"_api;
    pcreposix +=
        "src/pcre2posix.c",
        "src/pcre2posix.h";
    pcreposix.Public += pcre8;
//    pcreposix.Public += "PCRE2_LOCAL_WIDTH=8"_def;
    pcreposix.configureFile("src/config-cmake.h.in", pcreposix.BinaryPrivateDir / "config.h");
}
