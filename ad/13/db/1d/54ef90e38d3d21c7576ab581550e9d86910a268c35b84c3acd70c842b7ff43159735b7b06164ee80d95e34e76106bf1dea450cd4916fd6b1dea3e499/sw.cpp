...
struct MocCommand : Command
{
    ...
    MocCommand(const SwContext &swctx, NativeExecutedTarget &t)
        : Command(swctx), t(t)
    {
...
T &add_subproject(Target &base, const String &name)
{
    ...
    if (p.Local && fs::exists(p.SourceDir / qt_name))
...
{
    auto &qt = s.addProject("qtproject.qt", "5.13.0");
    ...
    auto write_tracepoints = [&tracegen](auto &t)
    {
        ...
        if (t.getSettings().TargetOS.Type == OSType::Windows)
            ...
            c << "lttng";
        c
            << cmd::in("qt" + t.getPackage().ppath.back() + ".tracepoints")
            << cmd::out("qt" + t.getPackage().ppath.back() + "_tracepoints_p.h")
            ;
    };
    ...
    // base
    {
        ...
        // tracegen
        {
            ...
            if (tracegen.getSettings().TargetOS.Type == OSType::Windows)
            {
