...
{
    auto &xerces = s.addTarget<LibraryTarget>("apache.xerces_c", "3.2.4");
    xerces += Git("https://github.com/apache/xerces-c", "v{v}");

    xerces.setChecks("xerces", true);

    xerces += "src/xercesc/.*"_rr;
    xerces -= "src/xercesc/.*\\.c"_rr;

    xerces.Public += "src"_id;

    xerces += "XERCES_BUILDING_LIBRARY"_d;
    xerces.Public += "XERCES_AUTOCONF"_d;
    xerces.Public += "XERCES_USE_MSGLOADER_INMEMORY"_d;
    if (xerces.getBuildSettings().TargetOS.Type == OSType::Windows)
    {
...
void check(Checker &c)
{
    ...
    s.checkIncludeExists("cstdint").setCpp();
