...
{
    auto &botan = s.addTarget<LibraryTarget>("randombit.botan", "2.11.0");
    ...
    if (botan.getSettings().TargetOS.Type != OSType::Macos)
    {
        botan -= ".*commoncrypto.*"_rr;
        botan -= ".*macos.*"_rr;
    }

    if (botan.getSettings().TargetOS.Type == OSType::Windows)
    {
    ...
    botan -= "prov/bearssl/.*"_rr;
    //botan.Public += "org.sw.demo.bearssl"_dep;
