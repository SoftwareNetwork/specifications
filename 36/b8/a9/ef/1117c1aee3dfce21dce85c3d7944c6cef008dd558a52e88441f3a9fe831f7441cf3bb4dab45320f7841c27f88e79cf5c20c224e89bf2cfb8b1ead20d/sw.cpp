void build(Solution &s)
{
    ...
    auto &t = s.addTarget<SharedLibrary>("khronos.vulkan.loader", "1.1.102");
    ...
    auto GenerateFromVkXml = [&t](const path &output, const path &dep)
    {
        auto c = t.addCommand();
        c << cmd::prog("org.sw.demo.python.exe-3"_dep)
            ...
            << "-registry"
            << cmd::in(t.getFile("org.sw.demo.khronos.vulkan"_dep, "registry/vk.xml"))
            << "-scripts"
            << t.getFile("org.sw.demo.khronos.vulkan"_dep, "registry")
            << output
            << cmd::end()
    ...
    t.Public += "org.sw.demo.khronos.vulkan"_dep;
}
